...
license_info(SpVer, Site, License) :-
        member(Where, [local,
                       reg(3),                  % HKEY_CURRENT_USER
                       reg(4)                   % HKEY_LOCAL_MACHINE
                      ]),
        License1 = product(SpVer,_Expiration,_Code),
        ( Where == local ->
           prolog:license_fact(site(Site)),
           prolog:license_fact(License1)
        ; Where = reg(RootCode) ->
           prolog:get_license_w32(RootCode, SpVer, Site, License1)
        ),
        %% [PM] 4.1.0 Note that we want to ignore the license-specific
        %% exceptions so error/2 would be wrong (but instead _Excp is
        %% too permissive SPRM 11617).

        %% [PM] 4.1.3 let SPIDER warn, we should fix our exception handling.
        %% Excp = _, % circumvent SPIDER unbound-analysis to avoid catch-all-warning
        catch(prolog:valid_license(Site, License1), Excp, fail),
        !,
        Excp = _, % [PM] 4.1.3 avoid singleton warning
        License = License1.
