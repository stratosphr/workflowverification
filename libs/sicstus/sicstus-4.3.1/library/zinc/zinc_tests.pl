...
can_not_run_on_32bits('grocery.fzn'). % [PM 4.2.1 Gets 'CLPFD integer overflow' on 32-bit
can_not_run_on_32bits('grocery.mzn'). % [PM 4.2.1 Gets 'CLPFD integer overflow' on 32-bit

can_run(File) :-
    running_on_32_bit,
    can_not_run_on_32bits(File),
    !,
    fail.
can_run(_File).
...
test(g12, [forall(file_member_of_directory(library('zinc/examples/G12'), '*.fzn', BaseName, F))]) :-
        ( can_run(BaseName) ->
	  fzn_run_file(F, [statistics(true), timeout(120000)])
        ; otherwise ->
	  format(user_error, '! Warning: Skipping test ~w (this is expected)', [BaseName])
        ).
