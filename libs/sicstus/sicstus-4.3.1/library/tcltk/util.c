...
#if 0
   struct event_stream_data * get_stream_data(SP_stream *stream, Tcl_Interp *interp)
   {
     spio_t_error_code code = SPIO_E_ERROR;
     void *user_data = NULL;

     code = SP_get_stream_user_data(stream, TCLTK_STREAM_CLASS, &user_data);
     if (SPIO_FAILED(code)
         ||
         ( user_data != NULL
           &&
           ((struct event_stream_data *)user_data)->interp != interp))
       {
         user_data = NULL;
       }

     return (struct event_stream_data *)user_data;
   }
#endif  /* 0 */
