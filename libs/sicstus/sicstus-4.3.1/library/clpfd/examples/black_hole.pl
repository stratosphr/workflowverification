...
:- module(black_hole, [black_hole/0]).
...
black_hole :-
	...
	neighbors(Table),
	(   fromto(Card,[Cardi,Cardj|Cards],[Cardj|Cards],[_]),
	    foreach([Cardi,Cardj],Tuples)
	do  true
	),
	table(Tuples, Table),
	% Link cards and positions
	assignment(Card, Pos, [consistency(bound)]),
	% A card must be played before the one under it.
	layout(Layout),
	(   foreach([L1,L2,L3],Layout),
	    param(Pos)
	do  nth1(L1, Pos, Pos1),
	    nth1(L2, Pos, Pos2),
	    nth1(L3, Pos, Pos3),
