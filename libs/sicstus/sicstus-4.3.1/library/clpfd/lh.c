	  ...
	  int closed)
{
  ...
  int back2 = *nback;
  int back3 = *nback;
  SP_integer lh0 = clh1(eps, sigma, &back0, ntask, lhs, lh_table, lch_table, row, bbheight, closed, 0x0);
  SP_integer lh1 = clh1(eps, sigma, &back1, ntask, lhs, lh_table, lch_table, row, bbheight, closed, 0x1); /* use poly_case */
  SP_integer lh2 = clh1(eps, sigma, &back2, ntask, lhs, lh_table, lch_table, row, bbheight, closed, 0x2); /* use too_big_loss */
  SP_integer lh3 = clh1(eps, sigma, &back3, ntask, lhs, lh_table, lch_table, row, bbheight, closed, 0x3); /* use poly_case, use too_big_loss */

  SP_ASSERT(back0<0 || back1<0 || back2<0 || back3<0 || (lh0==lh1 && lh1==lh2 && lh2==lh3));
  *nback = back3;
  return lh3;
}
#endif
