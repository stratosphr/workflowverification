...
#if VERBOSE_SPTI
  {
    fprintf(stderr, "Registing spti hook (\n");
    fflush(stderr);
  }
#endif  /* VERBOSE_SPTI */
...
#if VERBOSE_SPTI
      {
        fprintf(stderr, "SP_install_spti_hook() failed (0x%" SPRIxINTEGER ")\n", (SP_uinteger)code);
        fflush(stderr);
      }
#endif  /* VERBOSE_SPTI */
    }
...
static void unregister_spti(tSP_install_spti_hook *install_spti_hook)
{
  ...
  if (SPIO_FAILED(code = (*install_spti_hook)(spti_hook, NULL, SP_INSTALL_SPTI_HOOK_OPTION_UNINSTALL)))
    {
#if VERBOSE_SPTI
      fprintf(stderr, "SP_install_spti_hook(UNINSTALL) failed (0x%" SPRIxINTEGER")\n", (SP_uinteger)code);
      fflush(stderr);
#endif  /* VERBOSE_SPTI */
    }
