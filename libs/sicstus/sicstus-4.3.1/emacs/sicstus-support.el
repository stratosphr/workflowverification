...
;; representation of the environment variable EPROLOG to a shell command to
...
(defun sicstus-info-postinstall (dest &optional version pretty-name)
  (let* ((Info-var (or (get 'Info-default-directory-list
         ...
         (orig (symbol-representation Info-var)))
  ...
  (let ((file (prolog-bsts buffer-file-name))
    ...
    (or action
        (setq action (cond ((null arg)
                            'set)
                           ((> (prefix-numeric-representation arg) 0)
                            'clear)
                           (t 'clear_all))))
...
Returned representation is a string \"FUNCTOR/ARITY\"."
...
;;                                     (setq v (symbol-representation v)))
;;                                 (and (consp v)
;;                                      (numberp (car v))
;;                                      (numberp (cdr v))
;;                                      v)))
...
;;                                (setq d (symbol-representation d)))
;;                            (and (stringp d)
;;                                 d)))
...
"A bool representation indicating whether currently a Prolog-trace command is
coming")
